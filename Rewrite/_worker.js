...
// 解析 Base64 订阅内容（支持 nodejs-argo 生成的订阅）
async function parseBase64Subscription(url) {
    try {
        const response = await fetch(url, {
        ...
        for (const line of lines) {
            const nodeInfo = parseNodeLink(line.trim());
            if (nodeInfo) nodes.push(nodeInfo);
        }

        return nodes.length > 0 ? nodes : null;
    } catch (error) {
        console.error('解析 Base64 订阅失败:', error);
        return null;
    }
}
...
function parseNodeLink(link) {
    try {
        if (link.startsWith('vless://') || link.startsWith('trojan://')) {
            // 解析 VLESS/Trojan 格式: protocol://uuid@server:port?params#name
            const protocol = link.startsWith('vless://') ? 'vless' : 'trojan';
            const withoutProtocol = link.replace(/^(vless|trojan):\/\//, '');
            const [mainPart, fragment] = withoutProtocol.split('#');
            const [userAndServer, queryString] = mainPart.split('?');
            const [uuid, serverPart] = userAndServer.split('@');
            ...
            // 解析 VMess 格式: vmess://base64_json
            const base64Part = link.replace('vmess://', '');
            let jsonStr;
            try {
                // 处理可能包含中文的编码
                jsonStr = decodeURIComponent(atob(base64Part).split('').map(c =>
                    '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)
                ).join(''));
            } catch (e) {
                jsonStr = atob(base64Part);
            }
        ...
        console.error('解析节点链接失败:', error);
    }
    return null;
}

// 从解析的节点生成带优选 IP 的新节点
function generateLinksFromParsedNodes(parsedNodes, optimizedIPs, user) {
    const links = [];
    const CF_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
    const CF_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443];

    for (const node of parsedNodes) {
        // 使用第一个节点的 host/sni/path 作为模板
        const templateHost = node.host;
        const templateSni = node.sni;
        const templatePath = node.path;
        const templateUUID = user || node.uuid;

        for (const ipInfo of optimizedIPs) {
            const safeIP = ipInfo.ip.includes(':') ? `[${ipInfo.ip}]` : ipInfo.ip;
            const port = ipInfo.port || 443;
            const ispName = ipInfo.isp || ipInfo.name || ipInfo.ip;
            const nodeName = `${ispName}-${port}`;
            ...
            if (node.protocol === 'vless') {
                ...
                if (isTLS) {
                    params.set('sni', templateSni);
                    params.set('fp', 'chrome');
                }
                links.push(`vless://${templateUUID}@${safeIP}:${port}?${params.toString()}#${encodeURIComponent(nodeName)}`);
                ...
                links.push(`trojan://${templateUUID}@${safeIP}:${port}?${params.toString()}#${encodeURIComponent(nodeName + '-Trojan')}`);
        ...
        // 只使用第一个节点作为模板（通常 vless/vmess/trojan 共用相同的 host/path）
        break;
    }
...
async function handleSubscriptionRequest(request, user, customDomain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath) {
    ...
    let nodeDomain = customDomain || url.hostname;  // 用户输入的域名用于生成节点时的host/sni
    const target = url.searchParams.get('target') || 'base64';
    let wsPath = customPath || '/';

    // 先尝试解析 piu 中的 Base64 订阅，提取正确的域名和路径
    let parsedSubscriptionNodes = null;
    if (piu && piu.toLowerCase().startsWith('https://') && egi) {
        try {
            const parsedNodes = await parseBase64Subscription(piu);
            if (parsedNodes && parsedNodes.length > 0) {
                parsedSubscriptionNodes = parsedNodes;
                // 从订阅中提取正确的 host 和 path
                const firstNode = parsedNodes[0];
                if (firstNode.host) {
                    nodeDomain = firstNode.host;  // 使用订阅中的 Argo 隧道域名
                    console.log(`从订阅中提取域名: ${nodeDomain}`);
                }
                if (firstNode.path && firstNode.path !== '/') {
                    wsPath = firstNode.path;  // 使用订阅中的正确路径
                    console.log(`从订阅中提取路径: ${wsPath}`);
                }
            }
        } catch (error) {
            console.error('预解析订阅失败:', error);
        }
    }
    ...
    // GitHub优选 / 优选API / Base64订阅
    if (egi) {
        try {
            // 检查是否是优选API URL（以https://开头）
            if (piu && piu.toLowerCase().startsWith('https://')) {
                // 首先尝试解析为 Base64 订阅（nodejs-argo 格式）
                const parsedNodes = await parseBase64Subscription(piu);

                if (parsedNodes && parsedNodes.length > 0) {
                    // 成功解析为代理节点订阅，获取优选 IP 列表并重新生成节点
                    console.log(`成功解析 ${parsedNodes.length} 个节点，正在获取优选 IP...`);
                    ...
                    if (wetestIPs.length > 0) {
                        optimizedIPs.push(...wetestIPs);
                    }

                    // 如果优选 IP 不足，添加默认优选域名
                    if (optimizedIPs.length < 5) {
                        const defaultDomains = [
                            { ip: 'cloudflare.182682.xyz', isp: 'CF优选1' },
                            { ip: 'bestcf.top', isp: 'CF优选2' },
                            { ip: 'cf.0sm.com', isp: 'CF优选3' }
                        ];
                        optimizedIPs.push(...defaultDomains);
                    }

                    // 使用解析的节点模板和优选 IP 生成新节点
                    const newLinks = generateLinksFromParsedNodes(parsedNodes, optimizedIPs, user);
                    finalLinks.push(...newLinks);

                    console.log(`生成了 ${newLinks.length} 个优选节点`);
                } else {
                    // 不是 Base64 订阅，按原有逻辑处理优选 API
...
<body>
    <div class="container">
        ...
        <!-- 使用说明卡片 -->
        <div class="card" style="margin-bottom: 16px; background: linear-gradient(135deg, rgba(48, 209, 88, 0.1) 0%, rgba(0, 122, 255, 0.1) 100%);">
            <div style="font-size: 16px; font-weight: 600; margin-bottom: 12px; color: #30d158;">📘 使用说明</div>
            <div style="font-size: 14px; line-height: 1.8; color: #86868b;">
                <div style="margin-bottom: 8px;">
                    <strong style="color: #f5f5f7;">基础配置：</strong><br>
                    • <strong>域名</strong> - 填写您的 Argo 隧道域名<br>
                    • <strong>UUID</strong> - 需与您的代理服务配置一致
                </div>
                <div style="margin-bottom: 8px;">
                    <strong style="color: #f5f5f7;">🚀 订阅整合（推荐）：</strong><br>
                    • 在「GitHub优选URL」中直接填入 <span style="color: #30d158;">nodejs-argo / edgetunnel</span> 的订阅地址<br>
                    • 系统将自动解析订阅，提取隧道配置，生成大量优选节点
                </div>
                <div>
                    <strong style="color: #f5f5f7;">⚠️ 注意事项：</strong><br>
                    • 如果您的隧道仅支持 TLS，请开启「仅TLS节点」选项<br>
                    • 优选域名效果通常优于优选IP，建议同时启用
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="form-group" id="githubUrlGroup" style="margin-top: 12px;">
                <label>GitHub优选URL / 订阅地址（可选）</label>
                <input type="text" id="githubUrl" placeholder="留空则使用默认地址，或填入订阅URL" style="font-size: 15px;">
                <small style="display: block; margin-top: 6px; color: #86868b; font-size: 13px;">
                    支持以下格式：<br>
                    • 优选IP列表URL（TXT格式）<br>
                    • nodejs-argo / edgetunnel 等项目的订阅地址<br>
                    <span style="color: #30d158;">💡 填入订阅地址后，将自动提取隧道信息并生成优选节点</span>
                </small>
            </div>
